...
#   (effect size). This script is called by a 
#   python helper script titled 
#   'SpatialVarImportance_all_GCMS.py'. The 
#   helper script allows us to restart R for
#   for each gcm so that we dont run into 
#   memory limitations caused by R.
###############################################
...
suppressMessages(library(treeshap))
...
#### load data used to fit the models ####
...
future_clim <- rast(file.path(dir, 'future', 'gcm',
                              gcm, 'final_variables_2069_2099.tif')) %>%
...
# wrapper function so that the first argument is the raster instead of the model
# object
treeshap_wrap <- function(mat.x, mod.x) {
  if (all(is.na(as.matrix(mat.x)))) {
...
# loop over models producing shap values for each model
for (i in 1:10) {
  if (file.exists(file.path(dir, 'future', 'gcm',
                            gcm, 'spatial_importance',
